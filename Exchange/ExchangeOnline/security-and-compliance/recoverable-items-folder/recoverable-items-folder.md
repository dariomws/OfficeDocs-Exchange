...
After the deleted item retention period expires, the item is moved to the Purges subfolder and is no longer visible to the user. When the Managed Folder Assistant (MFA) processes the mailbox, items in the Purges subfolder are purged from Exchange Online.
